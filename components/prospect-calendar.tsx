...
export default function ProspectCalendar({ prospects }: { prospects: ProspectItem[] }) {
  ...
  const [selected, setSelected] = useState(localDateKey(now.toISOString())!)
  ...
  return (
    <section className="prospect-calendar-layout">
      ...
      <article className="panel-card prospect-day-panel">
        <div className="panel-heading"><div><span className="eyebrow">Agenda comercial</span><h3>{new Intl.DateTimeFormat('es-MX',{dateStyle:'long'}).format(new Date(`${selected}T12:00:00`))}</h3></div><strong>{selectedItems.length}</strong></div>
