...
export default async function CobranzaPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <>
    <header className="page-header"><div><span className="eyebrow">Control financiero</span><h1>Cobranza</h1><p>Semana operativa de lunes a domingo, siempre con horario de Hermosillo.</p></div><div className="header-actions"><a className="primary-button" href="#registrar-pago">+ Registrar pago</a></div></header>
    ...
    {!administrator ? <section id="registrar-pago" className="panel-card operational-payment-first"><div className="panel-heading"><div><span className="eyebrow">Acción principal</span><h3>Registrar pago</h3><p>Captura el pago y genera el recibo si el cliente lo solicita.</p></div></div><PaymentForm processes={financial.rows.filter(r=>r.balance>0).map(r=>{const c=Array.isArray(r.clients)?r.clients[0]:r.clients;return{id:r.id,service_name:r.service_name,client_name:c?.full_name||'Cliente',balance:r.balance}})}/></section> : null}
